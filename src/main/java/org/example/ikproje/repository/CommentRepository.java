...
public interface CommentRepository extends JpaRepository<Comment,Long> {
    ...
    @Query("SELECT new org.example.ikproje.view.VwComment(c.id,co.name, CONCAT(u.firstName,' ',u.lastName),c.managerPhoto,c.companyLogo,c.content) FROM Comment c JOIN User u ON u.id=c.companyManagerId JOIN Company co ON co.id=u.companyId where c.companyManagerId=?1")
    List<VwComment> findAllVwCommentByCompanyManagerId(Long companyManagerId);
