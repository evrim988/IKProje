...
public interface UserRepository extends JpaRepository<User, Long> {
	...
	@Query("SELECT new org.example.ikproje.view.VwCompanyManager(u.id,c.id,u.firstName,u.lastName,u.email,c.name,u.phone,u.avatarUrl,c.logo,u.userRole,a.region,a.city,a.district,a.neighbourhood,a.street,a.postalCode,a.aptNumber,ud.hireDate,ud.tcNo,ud.sgkNo,ud.birthDate) FROM User u JOIN Company c ON u.companyId=c.id JOIN UserDetails ud ON u.id=ud.userId JOIN Address a ON ud.addressId=a.id WHERE u.id=?1")
